function hImgAxes = imagine_v2(varargin)
...
SAp.sTITLE            = 'IMAGINE 2';        % Title of the figure
                ...
                switch get(hF, 'SelectionType')
                    ...
                    case 'normal' 
                        ...
                        for i = 1:length(SData)
                            ...
                            try
                                set(SAxes.hSegImg(iAxisInd), 'Position', [dPanelPos(3)*(dNewPos(1)) - dPos(3)/2, dPanelPos(4)*(dNewPos(2)) - dPos(4)/2, dPos(3), dPos(4)]);
                            end 
                    ...
                    case 'normal'
                        ...
                        for i = 1:length(SData)
                            ...
                            if iD(1) > SPref.dROTATION_THRESHOLD % Moved mouse to left
                                ...
                                SData(i).iSegmention = flipdim(permute(SData(i).iSegmention, [1 3 2]), 2);
                            ...
                            if iD(1) < -SPref.dROTATION_THRESHOLD % Moved mouse to right
                                ...
                                SData(i).iSegmention = flipdim(permute(SData(i).iSegmention, [1 3 2]), 3);
                            ...
                            if iD(2) > SPref.dROTATION_THRESHOLD 
                                ...
                                SData(i).iSegmention = flipdim(permute(SData(i).iSegmention, [3 2 1]), 3);
                            ...
                            if iD(2) < -SPref.dROTATION_THRESHOLD
                                ...
                                SData(i).iSegmention = flipdim(permute(SData(i).iSegmention, [3 2 1]), 1);
...
%%             
          if (iAxisInd) > 0 && (iAxisInd <= length(SPanels.hImgFrame))         % Update Corresponding Axis
                ...
                dThisImg = SData(iSeriesInd).iImg(:,:,iNewImgInd);
%   
                if(~isempty(SData(iSeriesInd).iSegmention))
                    iMask = SData(iSeriesInd).iSegmention(:,:,iNewImgInd);
                    % iMask(iMask == 0) = 255;
                    rgbiMask =  label2rgb(iMask); 
                   % get(SImg.hSegImg(iAxisInd))
                    dThisImg = imfuse(dThisImg,rgbiMask,'blend','Scaling','joint' );

                   try
                    set(SImg.hSegImg(iAxisInd), 'CData', rgbiMask); 
                   % set(SImg.hSegImg(iAxisInd), 'AlphaData', 0.2);  
                   % set( SAxes.hSegImg(iAxisInd), 'color', 'none');
                     delete(SAxes.hSegImg(iAxisInd));
                   end
                else 
                %    set(SImg.hSegImg(iAxisInd), 'AlphaData', 0);  
                %     set( SAxes.hSegImg(iAxisInd), 'color', 'none'); 
               end 
               
                set(SImg.hImg(iAxisInd), 'CData', dThisImg); 
                set(STexts.hImg2(iAxisInd), 'String', sprintf('%u/%u', iNewImgInd, size(SData(iSeriesInd).iImg, 3)));
          end
    ...
    function fWindowScrollWheelFcn(hObject, eventdata) %#ok<INUSL> % Don't use ~ for the sake of backwards compatibility
        % -----------------------------------------------------------------
        fChangeImage(eventdata.VerticalScrollCount);
        return
        %%%%%%
        %%%%%%
        %%%%%%
    ...
    function fFillPanels
        ...
        for i = 1:length(SPanels.hImgFrame)
            if iSeriesInd <= length(SData)
                ...
                if(~isempty(SData(iSeriesInd).iSegmention))
                    iMask = SData(iSeriesInd).iSegmention(:,:,SData(iSeriesInd).iActiveImage);
                        rgbiMask =  label2rgb(iMask); 
                      %  set(SImg.hSegImg(i), 'CData', rgbiMask); 
                       % set(SImg.hSegImg(i), 'AlphaData', 0.2);  
                end
                dXDim = size(iImg, 2);
                dYDim = size(iImg, 1);
                dPos = get(SPanels.hImg(i), 'Position');
%                 if(~isempty(SData(iSeriesInd).iSegmention))
%                     % dPos = get(SPanels.hSegImg(i), 'Position');
%                      set(SAxes.hSegImg(i), 'Position', [SData(iSeriesInd).dDrawCenter(1)*dPos(3) - ...
...
%                  end 
    ...
    function fCreatePanels(iSize)
        ...
        for i = 1:(iSize(1)*iSize(2));
            ...
            SImg.hImg(i) = imagesc(zeros(1, 'uint8'), ...
                'Parent'                , SAxes.hImg(i), ...
                'CDataMapping'          , 'scaled');
                        
              
            
%             SAxes.hSegImg(i) = axes(...
%                 'Parent'                , SPanels.hImg(i), ...
%                 'Units'                  , 'pixels', ...
%                  'Position'              , [10 10 10 10]);
%               %  'Color'                  , 'none' , ...
%                
%             
%              SImg.hSegImg(i) = imagesc(zeros(1, 'uint8'), ...
%                  'Parent'                , SAxes.hSegImg(i), ...
%                  'AlphaData', 0.2);
%              
...
%             axis(SAxes.hSegImg(i), 'off');
    ...
    function iPanels = fParseInputs(cInput)        
        ...
        sMode = 'DataOnly';
        if length(cInput) > 1
            if ischar(cInput{3}), sMode = 'DataName'; end % Input is given in pairs of data and title
        end
        ...
        switch sMode
            ...
            case 'DataName'
                for i = 1:floor(length(cInput)/3)
                    if ~ischar(cInput{i * 3}), error('Input data must be tripes  (image , segmentaion (or empty []) and string-name)'); end
                    fAddImageToData(cInput{i * 3 - 2},cInput{i * 3 - 1}, cInput{i * 3}, 'startup');
                end
        end
	...
	% * *
    % = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    function fAddImageToData(dImage, iSegmentImage , sName, sOrigin)
...
% % %             SData(iInd).dScale = dScale;
             ...
             SData(iInd).iSegmention = uint8(iSegmentImage);
%             [SData(iInd).iSegmention, ~] = fIm2uint8(iSegmentImage);
            % SData(iInd).iNumOfSegments =  max(iSegmentImage(:));
%              if ( SData(iInd).iNumOfSegments > 10 ) 
%                     seg_num = num2str(SData(iInd).iNumOfSegments);
%                      choice = questdlg(['There are more than 10 segmentations  ?'], ...
%                         'Are you sure ?', ...
%                                 'Yep','Nop');
%                      if (strcmp(choice,'Nop'))
%                             error('To many segmentation (more than 10)...'); 
%                      end
%              end
%              if ( SData(iInd).iNumOfSegments > 20 )
%                  error('To much segmentation (more than 20)...'); 
%              end
             
            [SData(iInd).iImg, SData(iInd).dScale] = fIm2uint8(dImage(:,:,:,i));
	...
	% * *
    % = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    function [dFOut, dScale] = fIm2uint8(dFOut)
...
% % % % %         
    ...
    function fZoom(iD, dPanelPos)  
         for i = 1:length(SData)    
            ...
            try
			set(SAxes.hSegImg(iAxisInd), 'Position', [dPanelPos(3).*SData(i).dDrawCenter(1) - dImageWidth/2, ...
                dPanelPos(4).*SData(i).dDrawCenter(2) - dImageHeight/2, ...
                dImageWidth, dImageHeight]);	
            end 
